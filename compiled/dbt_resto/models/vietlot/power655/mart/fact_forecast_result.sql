...
with forecast as (
  ...
  select * from vietlot_power655.mart.fact_forecast

  where   1 = 1

  

    and   forecast_date >= coalesce((select max(forecast_date) from vietlot_power655.mart.fact_forecast_result), '1900-01-01')
...
dim_box as (
  ...
  select * from vietlot_power655.mart.dim_box
