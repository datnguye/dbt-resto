...
with total_box as (
  ...
  select count(1) as no from vietlot_power655.mart.dim_box
...
weighting as (
  ...
  from    vietlot_power655.mart.fact_number as occurrence
...
select   concat(cast(current_timestamp as 
    date
),'-',number_value) as fact_key
...
where   1 = 1
  ...
  and   cast(current_timestamp as 
    date
) >= coalesce((select max(forecast_date) from vietlot_power655.mart.fact_forecast), '1900-01-01')
